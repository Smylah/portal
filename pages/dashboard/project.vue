...
export default defineComponent({
  ...
  setup(_, { root }) {
    ...
    const steps = computed(() => {
      ...
      const projectProgress = testState.detail.progress
      ...
      const recruit = {
        ...
        active: routeName === recruitRouteName,
          ...
          (testCreated && projectProgress !== 'Draft: Create'),
        disabled:
          routeName === projectRouteName ||
          projectProgress === 'Draft: Create' ||
          progressState === null,
      ...
      const result = {
        ...
        disabled:
          recruit.disabled ||
          [projectRouteName, recruitRouteName].includes(routeName),
        to: `/dashboard/project/view-result/responses/${testId}`,
      }

      return [create, recruit, result] as Step[]
    })
    ...
    const saveOnCtrlS = (evt: KeyboardEvent) => {
      if (
        (evt.ctrlKey || (root.$appState.os === 'mac' && evt.metaKey)) &&
        /^s$/i.test(evt.key)
      ) {
        evt.preventDefault()

        root.$store.dispatch('projectSuite/create/autosave')
      }
    }
...
<template>
  <div
    class="min-h-[calc(100%-64px)] relative"
    v-on="
      projectState.submitting || $route.name !== 'dashboard-project-:id'
        ? undefined
        : { keydown: saveOnCtrlS }
    "
  >
