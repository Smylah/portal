...
export default defineComponent({
  ...
  setup(_props, { emit, root }) {
    ...
    const isDisabled = computed(() => {
      return _props.disabled || root.$breakpoint.isMobile
    })
    ...
    const modelSync = computed({
      ...
      set(val: boolean) {
        if (typeof val === 'boolean' && !isDisabled.value) {
          toggleModel(val)
        }
      },
    })
    ...
    <Teleport to="overlay">
      ...
      >
        <div
          ...
          :class="[
            {
              'bg-base-on-surface text-base-surface dark:bg-dark-base-on-surface dark:text-dark-base-surface':
                !invert,
              'bg-surface-default dark:bg-dark-surface-default': invert,
            },
            contentClass,
          ]"
        >
