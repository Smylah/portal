...
export default defineComponent({
  ...
  setup(_props, { emit, slots, root }) {
    ...
    const bodyFocusController = (evt: FocusEvent) => {
      if (
        evt.target instanceof HTMLElement &&
        isFurthestDialog.value &&
        modelSync.value &&
        contentRef.value &&
        !contentRef.value.contains(evt.target)
      ) {
        contentRef.value.focus()
      }
    }
    ...
    const addBodyFocus = () => {
      document.body.addEventListener('focusin', bodyFocusController)
    }

    const cleanup = () => {
      root.$store.commit('app/removeFromDialogs', id.value)

      document.body.removeEventListener('focusin', bodyFocusController)
      // closeDialog()
    }
    ...
    onBeforeMount(() => {
      if (modelSync.value) {
        ...
        requestAnimationFrame(() => {
          ...
          if (
            contentRef.value &&
            !document.activeElement.isSameNode(contentRef.value) &&
            !contentRef.value.contains(document.activeElement)
          ) {
            contentRef.value.focus()
          }
        })
  ...
  <Teleport :to="to">
    ...
    >
      ...
      >
        <template v-if="!plain">
          ...
          <Transition>
            <div
              ...
              class="content pointer-events-auto"
              :class="[
                contentClass,
            ...
            >
              <!-- content header -->
              <header v-if="showHeader" class="header" :class="headerClass">
                <div
        ...
        <slot v-else v-bind="payload" ref="contentRef" />
      </div>
    </FadeTransition>
  </Teleport>
</template>
