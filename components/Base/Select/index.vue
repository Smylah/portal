...
export default defineComponent({
  ...
  setup(_props, { emit, root }) {
    ...
    const preventInputKeyEvents = (evt: KeyboardEvent) => {
      if (/arrow/.test(eventKey(evt))) {
        evt.preventDefault()
      }
    }
    ...
    >
      ...
      <template #default>
        <menu
          ...
          class="p-4 shadow-3 rounded-lg bg-surface-default overflow-y-auto hide-scrollbar show-scrollbar-on-hover"
...
.pseudo-focus[data-pseudo-focus]:not([data-disabled]) {
  @apply !bg-action-primary-hovered !text-white active:scale-[0.995] transition-transform;
}
