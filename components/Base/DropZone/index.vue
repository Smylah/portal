...
import { convertToByte, formatByte, sleep, uid, uuidv4 } from '~/utils'
import { SaveFiles, DeleteFiles } from '~/services/fileMgr'
import type { SaveFilesForm } from '~/server-middleware/routes/fileManager/saveFile'

export default defineComponent({
  name: 'DropZone',
  components: { Tooltip, FadeTransition, FLIPContainer, LoadingBar },
  ...
  props: {
    ...
    modelValue: {
      type: [Array, String] as unknown as () => string | File[] | string[],
      default: undefined,
    },
    multiple: Boolean,
    required: Boolean,
    ...
    autoUpload: Boolean,
    uploadPayload: {
      type: Object as () => SaveFilesForm['config'],
      default: () => ({}),
    },
  },
  emits: [
  ...
  setup(_props, { emit, attrs, root }) {
    const srPreviewLoaded = ref(false)

    const showUploadOverlay = ref(false)

    const id = ref(_props.id || uid())

    const getId = computed(() => _props.id || id.value)
    ...
    const manualModel = ref<typeof _props.modelValue>(_props.modelValue || [])

    const modelSync = computed({
      get() {
        if (typeof _props.modelValue !== 'undefined' && !_props.autoUpload) {
          return [_props.modelValue].flat()
        }
        return [manualModel.value].flat()
      },
      set(val: File[]) {
        if (typeof _props.modelValue !== 'undefined') {
          emit('update:modelValue', [val].flat())
        }
        manualModel.value = [val].flat()
      },
    })

    const getFiles = computed(() => {
      return modelSync.value.map((file: string | File) => {
    ...
    const onChange = async (evt: InputEvent) => {
      if (showUploadOverlay.value) {
        return
      }
      ...
      if (inputFiles) {
        ...
        for (const file of Array.from(inputFiles)) {
          if (
          ...
          } else if (_props.autoUpload) {
            showUploadOverlay.value = true

            const getFileNames = [uuidv4()]
            ...
            const oldFileNames = [...fileNames.value]

            const { error } = await SaveFiles(root.$axios, {
              config: {
                createdBy: root.$user.id,
                fileNames: getFileNames,
                ..._props.uploadPayload,
              },
              oldFileNames,
              files: [file],
            })
            ...
            if (error) {
              ...
              fileNames.value = _props.uploadPayload.fileNames || getFileNames
      ...
      if (files.length) {
        modelSync.value = _props.multiple
          ? ([...modelSync.value, ...files] as File[])
          : files

        srPreviewLoaded.value = false

        emit('on-change', modelSync.value)

        if (_props.autoUpload) {
    ...
    const removeFile = (index: number) => {
      modelSync.value = (modelSync.value as any[]).filter((_, i) => i !== index)

      if (_props.autoUpload && fileNames.value.length) {
        DeleteFiles(root.$axios, {
          fileNames: fileNames.value,
        }).then((x) => {
          emit('upload:delete', x.data)
        })
      }
    }
    ...
    const onSrPreviewLoad = () => {
      srPreviewLoaded.value = true
    }

    onMounted(() => {
      if (modelSync.value) {
        modelSync.value = [modelSync.value].flat()
      }
    })
...
<template>
  <FLIPContainer
    ...
    :view-port="viewPort"
    :disabled="disablePreview || plain || !getFiles.length"
    :enter-transition="{
      duration: '350',
      ...
      >
        ...
        >
          <div
            ...
            @click="openInput"
          >
            <label :for="getId" class="sr-only">Select file(s) </label>
          </div>
        ...
        <FadeTransition v-if="autoUpload">
          <div
            v-if="showUploadOverlay"
            class="pseudo bg-black/80 flex-centered !pointer-events-auto cursor-default"
            :title="uploadError ? undefined : 'Uploading...'"
          >
            <div
              class="relative w-[calc(100%-40px)] max-w-[400px]"
              :class="{ 'h-full': uploadError }"
            >
              <div
                class="absolute position-centered w-full"
                :class="{ 'h-full': uploadError }"
              >
                <div
                  v-if="uploadError"
                  class="grid gap-y-8 w-full h-full justify-items-center content-center"
                >
                  <span>
                    <Button
                      icon="RefreshMajor"
                      plain
                      class="!text-white fill-icon !bg-transparent"
                      @click="retryUpload"
                    >
                      Retry
                    </Button>
                ...
                <LoadingBar
                  ...
                  duration="20s"
                  :state="
                    uploadSuccess || !showUploadOverlay
                      ? 'finish'
                      : uploadError
                      ? 'error'
                      : 'start'
                  "
                  class="!absolute !h-4 !rounded-l-full"
                />
              </div>
            </div>
          </div>
        ...
        <div class="w-full h-80 flex-centered z-2 relative">
          ...
          >
            ...
            >
              ...
              <Tooltip v-slot="{ events }" invert label="Remove" class="h-full">
                <button
              ...
              <Tooltip v-slot="{ events }" invert label="Close" class="h-full">
                <button
